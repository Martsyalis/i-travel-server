    ...
    User.findById(req.user.id)
      .then(user => {
        if (!user.experiences.find(exp => exp.toString() === req.params.id)) {
          throw {
            code: 401,
            error: 'authentication failed, this is not your exp to post'
          };
        }
        const s3 = new AWS.S3();
    ...
    User.findById(req.user.id)
      ...
      .then(user => {
        if (!user.experiences.find(exp => exp.toString() === req.params.id)) {
          throw {
            code: 401,
            error: 'authentication failed, this is not your exp to delete'
          };
        }
        return User.findByIdAndUpdate(
          req.user.id,
          ...
          return Experience.findByIdAndRemove({ _id: req.params.id }).then(
            response => res.send({ removed: !!response })
          );
        });
      })
      .catch(next);
